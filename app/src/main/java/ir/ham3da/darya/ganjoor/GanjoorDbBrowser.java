...
//    }
     ...
     * @return
     */
    public int getPoemPoetId(int PoemId)
    {
        if (getIsConnected())
        {

            Cursor cursor = _db.rawQuery("SELECT c.poet_id " +
                    "From [poem] p " +
                    "INNER JOIN [cat] c ON c.id = p.cat_id " +
                    "WHERE p.id=" + PoemId + " Limit 1", null);
            if (cursor.moveToFirst())
            {

                cursor.moveToFirst();
                int poet_id = cursor.getInt(0);
                cursor.close();
                return poet_id;
            }
            cursor.close();
     ...
     * @return
     */
    public int getPoemCateId(int PoemId)
    {
        if (getIsConnected())
        {

            Cursor cursor = _db.rawQuery("SELECT cat_id From [poem] WHERE id=" + PoemId, null);
            if (cursor.moveToFirst())
            {
     ...
     * @return boolean
     */
    public boolean checkAudioFileExist(String fchecksum)
    {
        String dl_path = AppSettings.getAudioDownloadPath(mContext);
        File file = new File(dl_path + "/" + fchecksum + ".mp3");
        return file.exists();
    }
     ...
     * Check Audio File is Exist
     * @param fchecksum String checksum
     * @param dl_path file download path
     * @return boolean
     */
    public boolean checkAudioFileExist(String fchecksum, String dl_path)
    {
        File file = new File(dl_path + "/" + fchecksum + ".mp3");
        return file.exists();
    }
        ...
        if (!fileName.toLowerCase().endsWith(".gdb"))
        {
            ...
            try
            {
                ...
                while (entries.hasMoreElements())
                {
                    ...
                    {
                        File gdbFile = new File(new File(Objects.requireNonNull(new File(fileName).getParent())), entry.getName());
